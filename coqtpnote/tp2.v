...
Fixpoint eval (env : string -> nat) (e : exp) : nat :=  (* TODO *)
  match e with
  |Var n => env n
  |Const i => i
  |Add c i => eval env c + eval env i
  |Sub c i => eval env c - eval env i
...
(* Fill in the small step semantics below:
   ...
   - rule ssI: whe the instruction is a 'ifThen l1 l2' and the top of
     ...
     top of the stack. *)
Inductive smallStep : (list nat * asm) -> (list nat * asm) -> Prop := (* | TODO *)
  | ssPush : forall c stk l, smallStep (stk, push c l) (c :: stk, l)
  | ssAdd : forall a b stk l, smallStep (a::b::stk, add l) ((a+b)::stk,l)
  | ssSub: forall a b stk l, smallStep (a::b::stk, sub l) ((a-b)::stk,l)
  | ssI : forall c stk l1 l2, smallStep (c::stk, ifThen l1 l2) (stk,match c with 
                                                                |0 => l2
                                                                |_ => l1
                                                                 end).
...
Inductive bigStep : list nat -> asm -> list nat -> Prop :=
  ...
  | bsStep : forall stk stk' stk'' l l' , smallStep (stk,l) (stk',l') -> bigStep stk' l' stk'' -> bigStep stk l stk''.
...
Fixpoint compile_cnt (env : string -> nat) (e : exp) (cnt : asm) : asm :=(* TODO *)
  match e with 
  | Var n => push (env n) (cnt)
  | Const c => push c (cnt)
  | Add e1 e2 => compile_cnt env e2 (compile_cnt env e1 (add cnt))
  | Sub e1 e2 =>compile_cnt env e2 (compile_cnt env e1 (sub cnt))
  | IfThen c e1 e2 => compile_cnt env c (ifThen (compile_cnt env e1 cnt) (compile_cnt env e2 cnt))
end.
...
|add l' => match stk with 
         |a::b::stk' => asm_interp (a+b::stk') l'
         |_=> StackError
        end
|sub l' => match stk with 
         |a::b::stk' => asm_interp (a-b::stk') l'
         |_=> StackError
        end
|ifThen a b => match stk with
                |0::stk' => asm_interp stk' b
                |_ :: stk'=> asm_interp stk' a
                |_ => StackError
                end
end.
